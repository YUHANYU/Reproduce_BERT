...
r"""
TransformerBlock部分，也就相等于Transformer Encoder的一层
"""
...
class PositionWiseFeedForward(nn.Module):
    ...
    def __init__(self, d_ff, hidden_size):
        ...
        self.w_1 = nn.Conv1d(in_channels=hidden_size,
                             out_channels=d_ff,
                             kernel_size=1)  # kernel_size=1，权重w_1层

        self.w_2 = nn.Conv1d(in_channels=d_ff,
    ...
    def forward(self, x):
        ...
        ffn = gelu(self.w_1(x.transpose(1, 2)))  # max(0, x*w_1+b1) B*d_ff*L
...
# Transformer的Encoder的一层
class TransformerEncoderLayer(nn.Module):
    """
    transformer模型encoder部分的一层
    """
    def __init__(self, d_ff, hidden_size):
        super(TransformerEncoderLayer, self).__init__()
        self.multi_heads_attn = MultiHeadAttention()  # 子层一，多头注意力层
        self.feed_forward = PositionWiseFeedForward(d_ff, hidden_size)  # 子层二，按位置前馈层

    def forward(self, enc_seq, pad_mask, no_pad_mask):
        attn = self.multi_heads_attn(enc_seq, enc_seq, enc_seq, pad_mask)  # 子层一，多头注意力层计算 B*L*(h*d=d_model)
